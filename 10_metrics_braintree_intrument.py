...
braintree = pd.read_csv('10_metrics_braintree_intrument.csv')
...
for PIT in distinct_PIT:
    ...
    for CT in PIT_distinct_CT:
        ...
        final = final.append(pivoted.sum().rename((PIT,CT, 'Total')))
        final = final.append((sub_df_decline_pivoted.sum().rename((PIT,CT, 'Decline Ratio'))
                /pivoted.sum().rename((PIT,CT,'Decline Ratio'))*100).round(2).fillna(0).astype(str)+ '%')
    ...
    if PIT == 'Credit Card' or PIT == "Apple Pay Card" :
        ...
        final = final.append(PIT_total_df_approved.sum().rename((PIT+' Subtotal',' ', 'Approved')))
        final = final.append(PIT_total_df_declined.sum().rename((PIT+' Subtotal',' ', 'Declined')))
        final = final.append(sub_total_pivoted.sum().rename((PIT+' Subtotal',' ', 'Total')))
        final = final.append((PIT_total_df_declined.sum().rename((PIT+' Subtotal',' ', 'Decline Ratio'))
        /sub_total_pivoted.sum().rename((PIT+' Subtotal',' ','Decline Ratio'))*100).round(2).fillna(0).astype(str)+ '%')
...
final = final.append(pivoted_grand_approved.sum().rename(('Grand Total',' ', 'Approved')))
final = final.append(pivoted_grand_declined.sum().rename(('Grand Total',' ', 'Declined')))
final = final.append(pivoted_grand.sum().rename(('Grand Total',' ', 'Total')))
final = final.append((pivoted_grand_declined.sum().rename(('Grand Total',' ', 'Decline Ratio'))
        /pivoted_grand_approved.sum().rename(('Grand Total',' ','Decline Ratio'))*100).round(2).fillna(0).astype(str)+ '%')
        
# print(final)
final.to_excel('10_metrics_braintree_intrument_Report.xlsx')